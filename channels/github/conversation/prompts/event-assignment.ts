...
const githubNotificationAssignmentEventInstructions = [
  ...
  'During this initial assignment only, use the native `sessions` tool, when available, to personalize the current session. Call it with action "assign_owner", ownerType "agent", and ownerId set to the trusted OpenClaw agent ID supplied below. Then call it with action "patch" and a color. Omit sessionKey and targets from both calls so they affect only the current session.',
  ...
  'Session appearance is optional setup, independent of implementation or GitHub publication. If the tool is unavailable or a call fails, continue the normal assignment response without retries, permission changes, shell commands, or direct session-store edits. Do not repeat successful setup on a retry or overwrite later manual owner or color choices. Keep this setup out of the public GitHub response.',
