...
- Choose the model driver independently of the test folder: use direct assertions by default, strict AIMock when the real OpenClaw agent/tool loop matters but model judgment does not, and a live model only when provider transport, genuine model interpretation, or Codex-native tool behavior is itself under test.
- Keep general installed/runtime contracts in `examples/` and GitHub notification lifecycle contracts in `scenarios/`; keep model fixtures beside their owning flow and share only provider lifecycle machinery through `scripts/`.
...
- Keep general Leia examples in `pr-examples-tests.yml` and manually dispatched GitHub notification acceptance scenarios in `notification-tests.yml`. Scope shared test credentials to each workflow's final Leia execution step even though every matrix entry receives that step environment. Use strict AIMock for the `agent` and `github` examples; only `models`, `path`, and `security` select a live OpenAI model. Only the `env`, `credentials`, `git`, `github`, `routing`, and `tool` examples may consume `OP_SERVICE_ACCOUNT_TOKEN`. Notification scenarios may consume only credentials required by their owned flow; the current `issue-work-*` scenarios consume both. Git, GitHub, routing, notification, and tool flows must load account tokens from their declared 1Password Environments rather than workflow environment variables. The assignment mock must match the created issue title and body so a passing fixture proves bounded provider context reached the installed model turn.
