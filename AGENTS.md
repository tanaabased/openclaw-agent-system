...
- Treat model-facing `agent_system_*` tools as the direct agent-bound execution surface. Managed `git` and `gh` shims may also run as descendants of Gateway-hosted agent commands only when they redeem an opaque active-agent capability; keep explicit `tool` and all `credentials` routes operator-only.
