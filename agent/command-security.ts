...
import inspectAgentCommand, { type AgentOperatorInvocation } from './inspect-command.ts';
...
export const agentCommandSecurityGuidance =
  'Use native agent_system_* tools for direct Agent System-managed operations. Repository helpers may invoke packaged Agent System launchers for registered command routes; OpenClaw binds managed execution to the active agent and admits only its workspace, declared local repositories, and managed worktree root. Bare git and gh may use sanitized host executables outside that scope; strict managed launchers never fall back. Invalid authority and another known agent workspace remain denied. Never select another agent or directly invoke unbound openclaw agent-system tool or credentials routes through command tools. Request install or Doctor/status with agent_system_install or agent_system_doctor and obtain OpenClaw chat approval; never run them through command tools. Do not access another agent workspace, manifest, environment, credentials, or identity. If neither a native capability nor a managed helper route is available, stop and ask the operator.';
