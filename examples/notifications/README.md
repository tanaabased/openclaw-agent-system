# GitHub Notification Assignment Example

This scenario runs the prepared Agent System package in the default Gateway and
proves the installed GitHub assignment lifecycle. It establishes an authenticated
baseline, rejects a self-authored assignment, admits an approved human assignment,
creates one managed worktree and one local no-tools briefing session, adopts both
after restart, and retires without deleting either. Scenario setup creates and
updates uniquely named issues in `tanaabased/agent-system-test`; the notification
channel itself must never comment, push, or perform another outbound GitHub write.
...
# should prepare os-isolated notification and approved-actor workspaces
mkdir "$TMPDIR/agent-system-notifications"
mkdir "$TMPDIR/agent-system-notification-actor"
if test "$RUNNER_OS" = "Linux"; then
  agent_login=emoriwan
  agent_token=GH_TOKEN_EMORI
  actor_login=tanaabot
  actor_node_id=U_kgDODUXqww
  actor_token=GH_TOKEN_TANAABOT
else
  agent_login=tanaabot
  agent_token=GH_TOKEN_TANAABOT
  actor_login=emoriwan
  actor_node_id=U_kgDOEUqvpg
  actor_token=GH_TOKEN_EMORI
fi
sed \
  -e "s/agent-login-placeholder/$agent_login/g" \
  -e "s/AGENT_TOKEN_BINDING/$agent_token/g" \
  -e "s/actor-login-placeholder/$actor_login/g" \
  -e "s/ACTOR_NODE_ID_VALUE/$actor_node_id/g" \
  "$GITHUB_WORKSPACE/examples/notifications/agent.yaml" > "$TMPDIR/agent-system-notifications/agent.yaml"
sed \
  -e "s/actor-login-placeholder/$actor_login/g" \
  -e "s/ACTOR_TOKEN_BINDING/$actor_token/g" \
  "$GITHUB_WORKSPACE/examples/notifications/actor-agent.yaml" > "$TMPDIR/agent-system-notification-actor/agent.yaml"
printf '%s' "$agent_login" > "$TMPDIR/notification-agent-login"
printf '%s' "$actor_login" > "$TMPDIR/notification-actor-login"
...
# should complete one authenticated empty baseline before assignment delivery
cd "$TMPDIR/agent-system-notifications"
...
openclaw agent-system tool worktree --agent notification-data -- list | jq -e 'length == 0'
...
openclaw agent-system tool worktree --agent notification-data -- list github-1329940218 > "$TMPDIR/approved-worktrees.json"
...
session_key="$(cat "$TMPDIR/approved-session-key")"
work_id="issue-$issue_database_id"
openclaw agent-system tool worktree --agent notification-data -- list github-1329940218 | jq -e --arg workId "$work_id" '[.[] | select(.workId == $workId)] | length == 1'
openclaw sessions --agent notification-data --json | jq -e --arg key "$session_key" '[.sessions[]? | select(.key == $key)] | length == 1'
params="$(jq -cn --arg sessionKey "$session_key" '{sessionKey:$sessionKey,limit:50,maxChars:20000}')"
