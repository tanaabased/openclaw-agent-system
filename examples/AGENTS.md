...
- Use `openclaw-setup` for the shared isolated profile and packed Agent System plugin setup. Pass the prepared pack through `--agent-system-plugin`, omit `--model` for model-free scenarios, and pass a complete provider/model reference only when the scenario invokes a live agent.
