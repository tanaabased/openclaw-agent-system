...
- Keep `agent`, `diagnostics`, `github`, and the `credentials` cache checks on strict AIMock and `approval`, `codex`, `containment`, `models`, and `path` on a live model unless the contract or evidence changes.
