...
- Keep `agent`, `github`, and the `credentials` cache checks on strict AIMock and `approval`, `containment`, `models`, and `path` on a live model unless the contract or evidence changes. The `codex` example uses direct assertions without a model.
