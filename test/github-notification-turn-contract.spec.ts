  ...
  it('should compose one supported lifecycle mode event contract', () => {
    ...
    assert.equal(
      ...
      [
        ...
        '## Mode',
        "Use the configured Work capabilities only when the request needs them. When repository work is needed, use the prepared lifecycle worktree from structured context and keep changes there. When presenting a plan in Work mode, use active first-person language such as 'I'm going to' or 'I will' so it is clear you intend to carry out the plan to resolve the lifecycle item. A conversational question or acknowledgment should be answered directly without unnecessary tool use. Preserve the selected model and effort on ordinary follow-ups. For a demonstrated reasoning blocker after focused investigation, explain what failed and ask for a specific stronger configured profile through the existing clarification flow. Authentication failures, rate limits, slow tests, and missing requirements are not reasoning blockers. Do not self-switch unless the native harness supports both the requested model and effort and preserves them on follow-up; never claim a stronger profile ran without native confirmation.",
        '## Event',
