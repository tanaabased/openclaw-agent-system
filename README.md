...
`install` first checks global credential prerequisites, then reconciles the foundational agent and path lifecycle components followed by configured capabilities such as GitHub. The agent component requires `agent.name`, adds the OpenClaw agent when absent, and reconciles the manifest-owned name and optional avatar. The path component creates the workspace `bin/` directory and projects the configured executable path into supported OpenClaw and Codex command surfaces. If `environment.op` is declared, installation verifies stored access before any component mutates state; it does not use the process-token fallback, prompt, import the process token, or store credentials. Every component verifies the state it owns, and repeated installation reports an explicit unchanged outcome for each active component. An existing agent id bound to another workspace is reported as a conflict instead of being silently replaced.
