# GitHub Issue Work PR Handoff Scenario

This GitHub Actions-only scenario proves delivery pull-request creation and the
`pull-request-opened` handoff for an `issue` + `work` lifecycle. It
...
normalized pull-request shape, real private event turn, and deterministic issue
handoff. The separate `issue-work-pr-continuation` scenario owns source-affine
pull-request replies and closed-unmerged recovery. The same
...
cd "$TMPDIR/agent-system-notifications"
OPENCLAW_LOG_LEVEL=error openclaw agent-system tool gh -- api --method POST /user/keys -f "title=agent-system-pull-request-handoff-$GITHUB_RUN_ID-$GITHUB_RUN_ATTEMPT-$RUNNER_OS" -f "key=$(cat "$HOME/.ssh/big-test-bucket-ssh.pub")" --jq .id > "$TMPDIR/notification-ssh.key-id"
...
openclaw-notification-setup evidence \
  --model "$NOTIFICATION_MODEL" \
  --scenario pr-handoff \
  --expected-evidence "$GITHUB_WORKSPACE/scenarios/issue-work-pr-handoff/expected-evidence.json"
```
