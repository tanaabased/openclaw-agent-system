...
const [
  packageContents,
  manifestContents,
  codexManifestContents,
  nodeVersionContents,
  installedOpenClawContents,
] = await Promise.all([
  readFile('package.json', 'utf8'),
  readFile('openclaw.plugin.json', 'utf8'),
  readFile('.codex-plugin/plugin.json', 'utf8'),
...
const failures = pluginMetadataFailures(packageMetadata, manifest, codexManifest).map(
  ({ message }) => message,
);
