...
containment, operation policy, isolated SSH authentication, environment-bound
SSH commit and tag signing with optional local trusted verification. Encrypted
keys, passphrase delivery, and workstation integration with the 1Password
desktop SSH agent remain deferred.
...
| `delete`  | `allow`, `ask`, `deny` | `deny`  | Ref, reflog, or unreachable-object deletion            |
...
identity, OpenSSH readiness, isolated SSH authentication against GitHub, managed
clone preparation, idempotent path reuse, explicit Git execution in the returned
path, and non-forced removal on macOS and Ubuntu. Resolver variants and
containment edge cases remain direct unit tests because the existing Git scenario
does not invoke a model. The scenario includes one signed commit, one signed tag,
and local verification through a
checked-in public allowed-signers fixture; signing does not add a second
scenario solely for the same runtime boundary.
